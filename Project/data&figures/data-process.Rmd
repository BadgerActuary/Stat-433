...
unemployment = unemployment_raw %>% 
  ...
  separate(area_name, into = c("County", "State2"), sep = "\\s", extra = "merge") %>% 
  separate(State2, c("County2","State"), sep = ", ") %>% 
  pivot_wider(names_from = Attribute, values_from = Value) %>% 
  select(State, County, Unemployment_rate_2019) %>% 
  rename(Unemployment_Rate=Unemployment_rate_2019)

# Clean up education data
education = education_raw %>% 
  filter(str_detect(Area.name, "County")) %>% 
  separate(Area.name, into = c("County", "County2"), sep = "\\s", extra = "merge") %>%
...
population = population_raw %>% 
  filter(str_detect(Area_Name, "County")) %>% 
  separate(Area_Name, into = c("County", "County2"), sep = "\\s", extra = "merge") %>% 
...
HPI = HPI_temp %>% tail(-1) %>% 
  filter(Year == "2019"&`HPI with 2000 base`!=".") %>% 
  drop_na(`HPI with 2000 base`) %>% select(State, County, `HPI with 2000 base`) %>% 
  rename(HPI='HPI with 2000 base')
